...
/**
 * Reads a payload message of size msg_len from socket fd, storing it in
 * payload_out. Returns number of bytes read, or 0 for reading nothing, or -1
 * for error.
 *
 * @param fd            File descriptor to read from.
...
/**
 * Reads an entire packet from socket fd. Stores header in header_out and
 * payload in payload_out. Returns number of bytes read from payload, or 0 for
 * reading nothing, or -1 for error.
 *
 * @param fd            File descriptor to read from.
 ...
 */
ssize_t write_packet(int fd, const struct msg_header *header,
                     const char *payload, size_t msg_len);
 ...
 */
int create_header(struct msg_header *header_out, size_t msg_len,
                  uint8_t msg_type, bool msg_done);
