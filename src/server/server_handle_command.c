...
enum CMD_RES server_handle_command(struct server_state *state,
                                   struct client_info *user, char *cmd,
                                   char *saveptr) {
...
enum CMD_RES handle_list(struct server_state *state, struct client_info *user) {
  // Go through the client list, printing sending the names of each client to
  // the sender
...
enum CMD_RES handle_whisper(struct server_state *state,
                            struct client_info *user, char *saveptr) {
  ...
  uint8_t *formatted_whisper =
      format_whisper_message((const struct client_info *)user,
                             (const uint8_t *)whisper_msg, whisper_len);
  ...
  // Send to target
  if (client_send_direct_message(target->client_fd, formatted_whisper,
