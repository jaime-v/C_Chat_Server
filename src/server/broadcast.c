...
int broadcast(struct server_state *state, struct client_info *sender,
  ...
  for (size_t i = 0; i < state->client_count; ++i) {
    ...
    if (current_client != sender) {
      ...
      if (client_enqueue_msg_packet(current_client, packet, MSG_SIZE_LIMIT) ==
          -1) {
        free(packet->packet_data);
        free(packet);
      }

      if (current_client->epollout_enabled == 0 &&
          current_client->msg_queue.head == packet) {
